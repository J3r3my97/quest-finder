...
const mockSavedSearches: (SavedSearch & { alert?: Alert; matchCount: number })[] = [
  {
    id: "1",
    userId: "user1",
  ...
  },
  {
    ...
    filters: {
      ...
      naicsCodes: ["541519"],
      setAsideTypes: ["8a", "small_business"],
    },
...
function formatFilters(filters: SavedSearch["filters"]): string {
  ...
  if (filters.setAsideTypes?.length) {
    ...
    parts.push(filters.setAsideTypes.map((t) => labels[t] || t).join(", "));
  }
  return parts.join(" • ");
}
    ...
    setSearches((prev) =>
      prev.map((search) => {
        if (search.id === searchId && search.alert) {
          return {
            ...search,
            alert: { ...search.alert, enabled: !search.alert.enabled },
          };
        }
        return search;
      })
            ...
            <Card key={search.id} className="hover:border-primary/50 transition-colors">
              ...
              <CardContent>
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  ...
                  {search.alert?.lastSent && (
                    <span>Last alert: {formatDate(search.alert.lastSent)}</span>
                  )}
                </div>
              </CardContent>
            </Card>
